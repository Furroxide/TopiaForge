...
namespace Robotopia.Mods.UnityUi
{
    ...
    /// </summary>
    public sealed class QwHudLayer : QwContainer
    {
        private readonly GraphicRaycaster raycaster;
        private float hudScale = 1f;

        internal QwHudLayer(UiHost host, QwContainer canvasRoot)
            : base(host, QwScheme.Hud, canvasRoot.Go)
        {
        ...
        public QwFloaterLayer Floaters(int poolSize = 16)
        {
            return new QwFloaterLayer(World, poolSize, speechBubbles: false);
        }

        /// <summary>Pooled speech bubbles (world-projected, with backing panel).</summary>
        public QwFloaterLayer SpeechBubbles(int poolSize = 12)
        {
            return new QwFloaterLayer(World, poolSize, speechBubbles: true);
        }

        /// <summary>Center transient banner with the punch-hold-fade timeline.</summary>
        public QwBanner Banner()
        {
            return new QwBanner(Scaled);
        }
    }
    ...
    /// </summary>
    public sealed class QwBanner : QwWidget, IQwThemeAware
    {
            ...
            : base(parent.Host, parent.Scheme, parent.CreateChildGameObject("Banner"))
        {
            ...
            var font = QwFonts.For(QwTextStyle.Banner);
            if (font != null)
            {
                label.font = font;
            }
            ...
            var driver = Go.AddComponent<QwBannerDriver>();
            driver.Banner = this;
            Go.SetActive(false);
            ApplyTheme(Theme);
        }
    ...
    internal sealed class QwBannerDriver : MonoBehaviour
    {
        public QwBanner? Banner;
