...
{
    /// <summary>
    /// Pooled channel-based tween runner ticked by QwRuntime. Channels write directly
    ...
    public static class QwTween
    {
        ...
        public static void FadeTo(QwWidget target, float from, float to, float duration, QwEase ease = QwEase.OutQuad, Action? onDone = null)
        {
            target.EnsureCanvasGroup().alpha = from;
            Start(target, Channel.Alpha, from, to, duration, ease, onDone);
        }

        public static void ScaleTo(QwWidget target, float from, float to, float duration, QwEase ease = QwEase.OutCubic, Action? onDone = null)
        {
            target.Rect.localScale = new Vector3(from, from, 1f);
            Start(target, Channel.ScaleUniform, from, to, duration, ease, onDone);
        }

        public static void MoveX(QwWidget target, float from, float to, float duration, QwEase ease = QwEase.OutCubic, Action? onDone = null)
        {
            Start(target, Channel.OffsetX, from, to, duration, ease, onDone);
        }

        public static void MoveY(QwWidget target, float from, float to, float duration, QwEase ease = QwEase.OutCubic, Action? onDone = null)
        {
        ...
        public static void Cancel(QwWidget target)
        {
        ...
        private static void Start(QwWidget target, Channel kind, float from, float to, float duration, QwEase ease, Action? onDone)
        {
            ...
            {
                overflowLogged = true;
                QwLog.Warn("Tween pool exhausted (" + Capacity + "); applying end states immediately. A mod is animating an unusual number of widgets.");
            }
