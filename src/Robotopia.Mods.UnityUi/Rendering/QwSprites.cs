...
namespace Robotopia.Mods.UnityUi
{
    ...
    public static class QwSprites
    {
        ...
        public static Sprite Fill(QwRadius radius)
        {
            EnsureAtlas();
            return Cache["fill-" + (int)radius];
        }

        public static Sprite Ring(QwRadius radius, float thickness)
        {
            EnsureAtlas();
            var key = thickness >= QwTokens.BorderStrong ? "ring3-" : "ring2-";
        ...
        public static Sprite Icon(QwIcon icon)
        {
            EnsureAtlas();
        ...
        private static void EnsureAtlas()
        {
            ...
            foreach (var radius in Radii)
            {
                ...
                tiles.Add(("fill-" + r, size, borders, (x, y, s) => QwRoundedRectMath.FillCoverage(x, y, s, s, r)));
                tiles.Add(("ring2-" + r, size, borders, (x, y, s) => QwRoundedRectMath.RingCoverage(x, y, s, s, r, QwTokens.BorderStandard)));
                tiles.Add(("ring3-" + r, size, borders, (x, y, s) => QwRoundedRectMath.RingCoverage(x, y, s, s, r, QwTokens.BorderStrong)));
            }

            tiles.Add(("circle", CircleSize, Vector4.zero, (x, y, s) => QwRoundedRectMath.CircleCoverage(x, y, s)));
            foreach (QwIcon icon in Enum.GetValues(typeof(QwIcon)))
            {
                var captured = icon;
                tiles.Add(("icon-" + captured, IconSize, Vector4.zero, (x, y, s) => QwIconRaster.Coverage(captured, x, y, s)));
            }
            ...
            QwLog.Info("Sprite atlas generated (" + AtlasWidth + "x" + atlasHeight + ", " + tiles.Count + " tiles).");
        }
