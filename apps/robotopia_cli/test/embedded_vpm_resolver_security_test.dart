...
void main() {
  ...
  setUp(() {
    scratch = Directory.systemTemp.createTempSync('robotopia-vpm-bridge-test-');
  });
  ...
  test('embedded VPM package remains a bounded read-only detector', () {
    ...
    expect(resolver, contains('PackageIdPattern'));
    ...
    expect(resolver, contains('robotopia unity resolve'));
  ...
  test('template metadata and author guidance describe explicit recovery', () {
    ...
    expect(templateReadme, contains('robotopia unity resolve .'));
...
Directory _findRepositoryRoot() {
  ...
  while (candidate.parent.path != candidate.path) {
    final template = Directory(
      p.join(candidate.path, 'templates', 'Robotopia.UnityWorldTemplate'),
    );
  ...
  throw StateError('Could not locate the Robotopia repository root.');
}
