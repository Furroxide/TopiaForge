...
void main() {
  test('ugc go-live rejects a stale Automerge session before launch', () async {
    ...
    final created = await Process.run(
      ...
      ['run', 'robotopia', 'new', 'mod', 't.golive', '--dir', temp.path],
    ...
    final setup = await Process.run(
      Platform.resolvedExecutable,
      [
        p.join(packageRoot, 'bin', 'robotopia.dart'),
        'ugc',
        'setup',
        '--transport',
        'automerge',
    ...
    final session = File(
      p.join(environment['ROBOTOPIA_DATA_ROOT']!, 'ugc-session.json'),
...
''');
    ...
    final result = await Process.run(
      Platform.resolvedExecutable,
      [p.join(packageRoot, 'bin', 'robotopia.dart'), 'ugc', 'go-live'],
      workingDirectory: project,
      environment: environment,
    );
