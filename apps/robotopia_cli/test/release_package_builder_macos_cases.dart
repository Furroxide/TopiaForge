...
void _registerReleaseMacPackagingTests() {
  test('ad-hoc macOS dry-run signing disables hardened runtime', () async {
    ...
    await MacPackageSigner(
      ...
      isMacOS: true,
      environment: const {},
    ).signIfConfigured(p.join(temp.path, 'QuantumWorks.app'), temp.path);
    ...
    await MacPackageSigner(
      ...
      environment: {
        ...
        'MACOS_DEVELOPER_ID_APPLICATION':
            'Developer ID Application: QuantumWorks (TEAMID1234)',
      },
    ).signIfConfigured(p.join(temp.path, 'QuantumWorks.app'), temp.path);
  ...
  test('macOS packages dispatch to separate runnable AOT binaries', () async {
    ...
    final launcher = Directory(p.join(temp.path, 'QuantumWorks.app'));
    _writeFile(launcher, ['Contents', 'MacOS', 'QuantumWorks'], 'launcher');
    _writeFile(launcher, [
      'Contents',
      'Frameworks',
      'App.framework',
    ...
    final dispatcher = File(p.join(payload, 'robotopia')).readAsStringSync();
  ...
  test('macOS packaging rejects a single lipo-style CLI executable', () async {
    ...
    final launcher = Directory(p.join(temp.path, 'QuantumWorks.app'));
    _writeFile(launcher, ['Contents', 'MacOS', 'QuantumWorks'], 'launcher');
    final singleCli = File(p.join(temp.path, 'robotopia'))
