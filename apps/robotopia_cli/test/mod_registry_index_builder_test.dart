...
void main() {
  ...
  setUp(() async {
    temp = await Directory.systemTemp.createTemp('robotopia-registry-test-');
  });
    ...
    final builder = ModRegistryIndexBuilder(
      client: _FakeGitHubReleaseClient(
        releases: [
          _release(
            tagName: 'v2.0.0',
            assets: [_asset('cool.mod-1.1.0.robotopiamod', 'new')],
          ),
          _release(
            tagName: 'v1.9.0',
            assets: [_asset('cool.mod-1.0.0.robotopiamod', 'old')],
          ),
          _release(
            tagName: 'v2.1.0-draft',
            draft: true,
            assets: [_asset('cool.mod-9.9.9.robotopiamod', 'draft')],
          ),
          _release(
            tagName: 'v2.1.0-beta.1',
            prerelease: true,
            assets: [_asset('cool.mod-8.8.8.robotopiamod', 'beta')],
          ),
        ],
        assets: {
          'new': newBytes,
  ...
  test('directory mode emits relative or base-resolved URLs', () async {
    ...
    File(
      p.join(packages.path, 'cool.mod-1.0.0.robotopiamod'),
    ...
    expect(mods.single['downloadUrl'], 'cool.mod-1.0.0.robotopiamod');
    ...
    expect(
      ...
      'https://mods.example.com/files/cool.mod-1.0.0.robotopiamod',
    );
  });
  ...
  test('community entries merge, sort by id, and carry origin', () async {
    final packages = Directory(p.join(temp.path, 'packages'))..createSync();
    File(
      p.join(packages.path, 'zeta.mod-1.0.0.robotopiamod'),
  ...
  test('reserved community prefixes fail the build', () async {
    final packages = Directory(p.join(temp.path, 'packages'))..createSync();
    final entries = Directory(p.join(temp.path, 'registry'))..createSync();
    _writeCommunityEntry(entries, id: 'robotopia.fake', versions: ['1.0.0']);
...
void _writeCommunityEntry(
  ...
  for (final version in versions.reversed) {
    final result = buildRegistryEntry(
      ...
      downloadUrl: 'https://mods.example.com/$id-$version.robotopiamod',
