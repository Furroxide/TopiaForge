...
class RobotopiaReleasePolicy {
  RobotopiaReleasePolicy._({
  ...
  static RobotopiaReleasePolicy load(String repositoryRoot) {
    final file = File(p.join(repositoryRoot, 'release', 'release-policy.json'));
    final json = _readObject(file);
    if (json['schemaVersion'] != 1) {
      throw StateError('${file.path} must use schemaVersion 1.');
    }
    ...
    return RobotopiaReleasePolicy._(
...
class RobotopiaReleaseCatalog {
  RobotopiaReleaseCatalog._(this.releases);

  final List<RobotopiaReleaseCatalogEntry> releases;

  static RobotopiaReleaseCatalog load(String repositoryRoot) {
    final file = File(p.join(repositoryRoot, 'release', 'catalog.json'));
    final json = _readObject(file);
    if (json['schemaVersion'] != 1 || json['releases'] is! List) {
      throw StateError('${file.path} must contain a schemaVersion 1 catalog.');
    }
    final releases = <RobotopiaReleaseCatalogEntry>[];
    final versions = <String>{};
    final tags = <String>{};
    for (final value in json['releases'] as List) {
      final entry = RobotopiaReleaseCatalogEntry.fromJson(
    ...
    return RobotopiaReleaseCatalog._(List.unmodifiable(releases));
  }

  RobotopiaReleaseCatalogEntry release(String version) {
...
class RobotopiaReleaseCatalogEntry {
  const RobotopiaReleaseCatalogEntry({
  ...
  factory RobotopiaReleaseCatalogEntry.fromJson(Map<String, Object?> json) {
    return RobotopiaReleaseCatalogEntry(
...
class ReleasePolicyValidator {
  ...
  Future<List<String>> validate({
    required RobotopiaReleasePolicy policy,
    required RobotopiaReleaseCatalogEntry release,
  ...
  void _validateCatalog(
    RobotopiaReleaseCatalogEntry release,
    String root,
    List<String> issues,
    ...
    final expected = <String>{
      'QuantumWorks-linux-x64.zip',
      'QuantumWorks-macos-universal.zip',
      'QuantumWorks-windows-x64.zip',
      for (final entry in release.mods.entries)
        '${entry.key}-${entry.value}.robotopiamod',
    };
  ...
  void _validateToolchains(
    ...
    if (!props.contains(
      '<RobotopiaDotNetRuntimeVersion>${policy.toolchains['dotnetRuntime']}</RobotopiaDotNetRuntimeVersion>',
    )) {
  ...
  void _validateComponents(
    ...
    final pubspecs = {
      'cli': 'apps/robotopia_cli/pubspec.yaml',
      'launcher': 'apps/robotopia_launcher_flutter/pubspec.yaml',
    ...
    final vpmPaths = {
      'com.robotopia.vpm-resolver':
          'templates/Robotopia.UnityWorldTemplate/Packages/com.robotopia.vpm-resolver/package.json',
      'com.robotopia.world-companion':
          'templates/Robotopia.UnityWorldTemplate/Packages/com.robotopia.world-companion/package.json',
      'com.robotopia.ugc-companion':
          'templates/unity-companion/Packages/com.robotopia.ugc-companion/package.json',
    };
    ...
    final assemblies = {
      'src/Robotopia.ModManager/Robotopia.ModManager.csproj':
          release.components['loader'],
      'src/Robotopia.ModManager.Core/Robotopia.ModManager.Core.csproj':
          release.components['loaderCore'],
      'src/Robotopia.Mods.Abstractions/Robotopia.Mods.Abstractions.csproj':
          release.components['sdk'],
      'src/Robotopia.Mods.UnityUi/Robotopia.Mods.UnityUi.csproj':
          release.components['qwui'],
      'src/Robotopia.GameCompat.Extractor/Robotopia.GameCompat.Extractor.csproj':
          release.components['gameCompatExtractor'],
      'src/Robotopia.GameCompat.Surface/Robotopia.GameCompat.Surface.csproj':
    ...
    final versionSource = readBoundedTextFileSync(
      File(p.join(root, 'src/Robotopia.ModManager.Core/RobotopiaVersions.cs')),
      maxBytes: CliFileLimits.metadata,
    );
    for (final entry in {
  ...
  void _validateMods(
    ...
    for (final directory in listBoundedDirectorySync(
      ...
      final file = File(p.join(directory.path, 'robotopia.mod.json'));
