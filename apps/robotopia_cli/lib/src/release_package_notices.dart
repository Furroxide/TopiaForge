...
class ReleasePackageNoticeWriter {
  ...
  void copyDartCliNotices(String destinationRoot) {
    ...
    File(p.join(destination.path, 'VERSIONS.json')).writeAsStringSync(
      '${const JsonEncoder.withIndent('  ').convert({'schemaVersion': 1, 'dartSdk': readBoundedTextFileSync(dartVersionFile, maxBytes: CliFileLimits.session).trim(), 'packages': versions})}\n',
      flush: true,
    );
  }
