...
class ReleaseMetadataInventoryBuilder {
  ...
  Future<Map<String, Object?>> _ecosystemInventory(
    ...
    for (final archiveName
        in release.artifacts
            .where((name) => name.startsWith('QuantumWorks-'))
            .toList()
          ..sort()) {
  ...
  Future<Map<String, Map<String, Object?>>> _readEcosystemFromArchive(
    ...
    for (final entry in archive.files) {
      ...
      if (!expectedPaths.contains(path)) {
        if (path.endsWith('.robotopiamod') || path.startsWith('dist/vpm/')) {
          throw StateError(
  ...
  Future<Map<String, Object?>> _provenanceInventory(
    ...
    final qwuiManifest = File(
      p.join(
        root,
        'src',
        'Robotopia.Mods.UnityUi',
        'Assets',
        'quantumworks-ui.manifest.json',
      ),
    );
    final qwui = _readBoundedJsonObject(qwuiManifest);
    final qwuiBundle = File(
      p.join(p.dirname(qwuiManifest.path), 'quantumworks-ui.bundle'),
    );
    _requireRegularFile(qwuiBundle, maxBytes: 64 * 1024 * 1024);
    final bundleHash = await _sha256File(qwuiBundle);
    if (qwui['editorVersion'] != policy.toolchains['unity'] ||
        qwui['sha256'] != bundleHash) {
      throw StateError(
        'QwUi provenance does not match the pinned editor or bundle.',
      );
    }
    return {
      'unity': {
        'editorVersion': qwui['editorVersion'],
        'bundle': qwui['bundle'],
        'bundleSha256': bundleHash,
        'manifest': p.relative(qwuiManifest.path, from: root),
        'manifestSha256': await _sha256File(qwuiManifest),
      },
      'bepInEx': {
