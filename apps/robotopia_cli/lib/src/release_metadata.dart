...
class RobotopiaReleaseMetadataBuilder {
  const RobotopiaReleaseMetadataBuilder();
  ...
  Future<ReleaseMetadataResult> build({
    ...
    final policy = RobotopiaReleasePolicy.load(repositoryRoot);
    final release = RobotopiaReleaseCatalog.load(
      repositoryRoot,
    ).release(version);
    ...
    final bom = <String, Object?>{
      r'$schema':
          'https://raw.githubusercontent.com/furroxide/quantum-works/main/schemas/robotopia.release-bom.schema.json',
      'schemaVersion': 1,
    ...
    _validateSchema(
      repositoryRoot,
      'schemas/robotopia.release-bom.schema.json',
      bom,
    );
    ...
    _validateSchema(
      repositoryRoot,
      'schemas/robotopia.release-spdx.schema.json',
      sbom,
    );
