...
/// `robotopia new ...` — project scaffolding (mods and Unity world projects), split out of
/// robotopia.dart to keep the entry file under the file-size cap.
extension _NewCommands on _RobotopiaCli {
  Future<int> _new(List<String> args) async {
    if (args.firstOrNull == 'unity-world') {
      ...
      if (args.contains('--live-sync')) {
        ...
        stdout.writeln(
          'Live sync seeded ($seed, watch folder $watch). Launch it with `robotopia ugc dev --project "$projectPath"`.',
        );
      }
      return 0;
    }
    ...
    stdout.writeln(
      'Next: edit robotopia.mod.json (or use `robotopia mod set|add|remove`), then validate with '
      '`robotopia check package ${workspace.projectRoot}`.',
    );
    if (options.template == 'world') {
      ...
      if (unityProject != null) {
        ...
        stdout.writeln(
          ...
          '`robotopia new unity-world <Name> --mod ${workspace.projectRoot}` '
          '(or `robotopia world link`), then build its bundle with `robotopia world build`.',
        );
      }
    }
    ...
    if (options.liveSync != null) {
      stdout.writeln(
        ...
        '`robotopia ugc setup` and start the full loop with `robotopia ugc dev`.',
      );
    }
    return 0;
  }

  /// Parses every scaffold-time manifest flag of `new mod`. `hashes` (pack-time), `legacy*` (migration-time),
  /// and `schemaVersion` (pinned to 2) are intentionally not flags.
