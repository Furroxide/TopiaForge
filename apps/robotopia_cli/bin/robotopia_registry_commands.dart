...
extension _RegistryCommands on _RobotopiaCli {
  Future<int> _registry(List<String> args) async {
    ...
    return switch (command) {
      ...
      _ => throw UsageError(
        'Usage: robotopia registry index|add-entry|validate ...',
      ),
    };
  }
  ...
  Future<int> _registryIndex(List<String> args) async {
    ...
    if (output == null || (repository.isEmpty == packagesDir.isEmpty)) {
      throw UsageError(
        'Usage: robotopia registry index (--repository owner/name | --dir packages) '
  ...
  Future<int> _registryAddEntry(List<String> args) async {
    ...
    if (packagePath == null || url == null) {
      throw UsageError(
        'Usage: robotopia registry add-entry <package.robotopiamod> '
