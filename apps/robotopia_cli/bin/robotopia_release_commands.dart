...
extension _RobotopiaReleaseCommands on _RobotopiaCli {
  Future<int> _release(List<String> args) async {
    return switch (args.firstOrNull) {
      ...
      _ => throw UsageError(
        'Usage: robotopia release build-package|test-package|validate-policy|build-metadata|verify-metadata ...',
      ),
    };
  }
  ...
  Future<int> _releaseValidatePolicy(List<String> args) async {
    ...
    final policy = RobotopiaReleasePolicy.load(root);
    final version = _option(args, '--version') ?? policy.productVersion;
    final release = RobotopiaReleaseCatalog.load(root).release(version);
  ...
  Future<int> _releaseBuildMetadata(List<String> args) async {
    ...
    final result = await const RobotopiaReleaseMetadataBuilder().build(
      repositoryRoot: root,
      version: version,
      targetSha: targetSha,
  ...
  Future<int> _releaseVerifyMetadata(List<String> args) async {
    final root = _releaseRepositoryRoot();
    final assets = _requiredReleaseOption(args, '--assets');
    await const RobotopiaReleaseMetadataBuilder().verify(
      repositoryRoot: root,
  ...
  String _requiredReleaseOption(List<String> args, String option) {
    ...
    if (value == null || value.trim().isEmpty) {
      throw UsageError(
        'Usage: robotopia release build-metadata|verify-metadata '
