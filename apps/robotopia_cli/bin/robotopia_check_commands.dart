...
/// `robotopia check ...` — validation for projects, packages, and registry
/// entries (split out of robotopia.dart for the file-size cap).
extension _RobotopiaCheckCommands on _RobotopiaCli {
  static const _checkPackageUsage =
      'Usage: robotopia check package <path> [--sha256 hex] '
  ...
  Future<int> _check(List<String> args) async {
    return switch (args.firstOrNull) {
      ...
      _ => throw UsageError('Usage: robotopia check project|package [path]'),
    };
  }
  ...
  Future<int> _checkPackage(List<String> args) async {
    ...
    if (FileSystemEntity.isDirectorySync(path)) {
      final file = File(p.join(path, 'robotopia.mod.json'));
      if (!file.existsSync()) {
        stderr.writeln('robotopia.mod.json was not found in $path.');
        return 1;
      }
    ...
    if (expectedSha != null) {
      if (package == null) {
        throw UsageError('--sha256 only applies to a packed .robotopiamod.');
      }
      ...
      issues.add(
        LauncherIssue(
          ...
          message:
              ...
              '`robotopia registry add-entry` against this package.',
        ),
      );
      return issues;
    }
      ...
      issues.add(
        LauncherIssue(
          ...
          message:
              ...
              'Re-run `robotopia registry add-entry` against the exact file '
              'you host.',
        ),
      );
    }
  ...
  Future<bool> _printResolvePlan(
    ...
    final plan = const DependencyPlanner().previewInstall(
      ...
      loaderVersion: RobotopiaRuntimeVersions.loaderVersion,
      sdkVersion: RobotopiaRuntimeVersions.sdkVersion,
    );
    stdout.writeln('');
