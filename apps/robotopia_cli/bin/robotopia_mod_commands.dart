...
/// `robotopia mod ...` — manifest management after creation. Every schema-v2 field of `robotopia.mod.json` can
/// be shown and edited from the terminal; edits are validated on write (same rules as `check package`).
extension _RobotopiaModCommands on _RobotopiaCli {
  ...
  Future<int> _mod(List<String> args) async {
    ...
    switch (sub) {
      ...
      default:
        ...
        stdout.writeln('  robotopia mod show [--project path]');
        stdout.writeln(
          '  robotopia mod set <field> <value> [--project path]      '
        ...
        stdout.writeln(
          '  robotopia mod add|remove dependency|optional-dependency|conflict <id[@range]> [--project path]',
        );
        stdout.writeln(
  ...
  Future<int> _mutateManifest(
    ...
    stdout.writeln('Updated robotopia.mod.json.');
    _printIssues(issues);
    return 0;
  }
