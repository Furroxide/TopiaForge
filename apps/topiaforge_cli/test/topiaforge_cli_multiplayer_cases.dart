...
void _multiplayerCliTests(_CliTestHarness Function() currentHarness) {
  test(
    'mod add multiplayer keeps the current schema and scaffolds the contract lock',
    () async {
      ...
      expect(
        migrateNoOp.stdout,
        contains('supported schema V${ModManifest.currentSchemaVersion}'),
      );
    },
  );

  test(
    'mod remove multiplayer keeps the project standalone on the current schema',
    () async {
      final created = await currentHarness().runCli([
        'new',
        'mod',
