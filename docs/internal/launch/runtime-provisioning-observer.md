...
The [original-process owner](../../../tools/TopiaForge.Acceptance.Windows/ProvisioningProcess.cs) creates only the fixed QA `Robotopia.exe`, suspended, with `-batchmode -nographics -noaudio` followed by one computed `-logFile` argument naming `outputRoot\<requestId>\player.log`. The broker derives that destination, validates it as a physical path inside the run directory and records it in the launch receipt; the launch input still accepts no arbitrary arguments. A one-process, kill-on-close job is assigned atomically at creation. It corroborates the original process handle, executable, native creation token and primary identity before resuming. No command interpreter, inherited handles, child processes, device actuation, capture or microphone is requested. Reviewed input files remain read-locked.
...
| Same run directory, `player.log` | Unity player log retained through the computed `-logFile` argument (diagnostic launch mode) |
| Same run directory, `runtime-diagnostics.json` | Read-only process snapshots (CPU, handles, memory, threads, windows, TCP/UDP owner tables, player-log facts) at 15, 45 and 75 seconds and before any termination |
| Same run directory, `provisioning-result.json` | Outcome, original exit/force-termination facts, observation digest, runtime facts and failures |
| `outputRoot\sandbox-recovery-required.json` | Persistent ownership/recovery marker |

The runtime observation kind is `sandbox-provisioning-runtime-observation-v1`; result kind is `sandbox-provisioning-result-v2` (schema 2), whose `runtime` object records `launchArguments`, `deadlineSeconds`, `runtimeMilliseconds`, `originalProcessExitCode`, the `playerLog` facts (`path`, `exists`, `bytes`, `sha256`) and `diagnosticsPath`. All carry `isolationAdmitted: false` and `qualifiesRelease: false`; the runtime observation also records `managerInitialized: false`. No acceptance acknowledgement or release evidence is produced. Early refusal can occur before a run directory exists; missing output never means success.
...
The v4 diagnostic launch mode adds exactly one computed `-logFile` argument (the run directory's `player.log`) to the unchanged fixed headless flags, takes read-only process, thread, handle, memory, socket and window snapshots at 15, 45 and 75 seconds and before any termination, and writes them to `runtime-diagnostics.json` before the result. The 90-second deadline, unforced-exit success condition, graphics mode, identity, inventory, firewall and ownership contracts are unchanged. The loader stamps every `Provisioning:` phase with UTC time and also reports the plugin's Unity `OnDestroy`.

Fixture verification: `tools/run-sandbox-fixture-shutdown.ps1` built a minimal Unity `6000.0.23f1` player that quits from its first `Update`, then ran it through the same launch, log and wait code path (`--fixture-shutdown`). It exited unforced with code 0 after 679 ms and retained a 2,135-byte player log despite `-nographics`. This proves that the mechanism retains Unity output and that a clean player exits promptly. It says nothing about the game's own shutdown, which runs a different Unity revision and its own quit tasks.

Source checkpoint: the private receipt `runtime-observer-shutdown-diagnostics-v4/final-source-receipt-v4.json` records 44 source files, the seven required C# validation logs (solution build with 0 warnings and 0 errors; manager, runtime, analyzers, generators and multiplayer harnesses; 53 focused observer checks), 100 launcher-contract checks, the published broker bundle v3 (192 files, passing the 29 broker and 17 waveform checks), the thirteen rebuilt loader hashes and the fixture evidence. The exact worktree content is snapshotted at `refs/snapshots/rc1-automation-worktree-v4`. The first runtime-harness attempt failed with exit code 127 (the worktree lacked its Flutter SDK link, then its locked pub dependencies) and is preserved in `checks-attempts.jsonl`; the rerun with a short process-local PATH passed.
