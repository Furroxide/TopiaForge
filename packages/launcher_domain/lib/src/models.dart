import 'dart:convert';

import 'versioning.dart';
import 'launch_resolution.dart';
import 'package:unorm_dart/unorm_dart.dart' as unicode;

part 'models/common_models.dart';
part 'models/developer_environment_models.dart';
part 'models/developer_vpm_models.dart';
part 'models/developer_models.dart';
part 'models/developer_unity_models.dart';
part 'models/install_models.dart';
part 'models/game_install_discovery_models.dart';
part 'models/package_inbox_models.dart';
part 'models/json_helpers.dart';
part 'models/ecosystem_identifier_contracts.dart';
part 'models/retired_identifier_contract.dart';
part 'models/manifest_support_models.dart';
part 'models/manifest_contract_constants.dart';
part 'models/manifest_schema_dispatch.dart';
part 'models/manifest_structural_validation.dart';
part 'models/manifest_contribution_structure.dart';
part 'models/manifest_contribution_shape_checks.dart';
part 'models/manifest_contribution_validation.dart';
part 'models/manifest_contribution_target_validation.dart';
part 'models/manifest_contribution_references.dart';
part 'models/manifest_contract_validation.dart';
part 'models/manifest_multiplayer_models.dart';
part 'models/manifest_multiplayer_validation.dart';
part 'models/manifest_contribution_models.dart';
part 'models/manifest_common_reader.dart';
part 'models/manifest_v6_reader.dart';
part 'models/manifest_models.dart';
part 'models/manifest_license_validation.dart';
part 'models/manifest_parsers.dart';
part 'models/runtime_dependency_models.dart';
part 'models/scaffold_contracts.dart';
part 'models/registry_index_models.dart';
part 'models/source_models.dart';
part 'models/profile_models.dart';
part 'models/profile_json_parsing.dart';
part 'models/world_authoring_models.dart';
part 'models/update_models.dart';

part 'models/launch_selection_models.dart';
part 'models/launch_preview_models.dart';
part 'models/launch_activity_models.dart';
