...
{
    internal static class ManifestV6Tests
    {
        ...
        {
            SchemaDispatchAllowsStandaloneV6();
            V6SessionMetadataRoundTrips();
            V6ModesHaveClosedSemantics();
            ...
            Console.WriteLine("ManifestV6Tests passed.");
        }

        private static void SchemaDispatchAllowsStandaloneV6()
        {
            ...
            Assert(standalone.IsStandaloneOnly && !standalone.DeclaresMultiplayer,
                "V6 without multiplayer metadata must remain standalone-only");
            Assert(ManifestValidator.Validate(standalone).Count == 0,
                "a canonical standalone V6 manifest must be valid");
            Assert(ManifestSchemaDispatch.Resolve(ModManifest.ManifestV6SchemaVersion) == ManifestSchemaContract.V6,
                "the V6 selector must dispatch to its dedicated reader");
            AssertThrows<InvalidDataException>(() => ModManifestJson.Deserialize(ManifestJson(7, string.Empty)),
                "unknown versions must fail without interpreting current fields");
            var errors = ManifestValidator.Validate(new ModManifest { SchemaVersion = 7 });
            Assert(errors.Count == 1 && errors[0].Contains("schemaVersion must be 6"),
                "future model versions must fail before current fields are interpreted");
            AssertThrows<InvalidDataException>(() => ModManifestJson.Deserialize(ManifestJson(
                6, ",\"worldGamemodes\":[]")), "V6 must reject even an empty retired worldGamemodes field");
        }

        private static void V6SessionMetadataRoundTrips()
        {
            ...
            Assert(manifest.DeclaresMultiplayer && !manifest.IsStandaloneOnly,
                "a valid V6 declaration must opt into multiplayer");
        ...
        private static void V6ModesHaveClosedSemantics()
        {
        ...
        {
            var packageRoot = Path.Combine(root, "manifest-v6-package");
        ...
        {
            var paths = new ManagerPaths(Path.Combine(root, "manifest-v6-install", "BepInEx"));
            paths.EnsureCreated();
            var state = new ManagerState();
            var content = System.Text.Encoding.UTF8.GetBytes("{\"difficulty\":4}");

            var missingHashPackage = Path.Combine(root, "manifest-v6-missing-hash.topiaforgemod");
            ...
            var validPackage = Path.Combine(root, "manifest-v6-valid.topiaforgemod");
        ...
        private static void AssertThrows<T>(Action action, string message) where T : Exception
        {
            ...
            throw new InvalidOperationException("Manifest V6: " + message);
        }
