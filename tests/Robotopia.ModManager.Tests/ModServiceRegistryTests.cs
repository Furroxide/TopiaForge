...
namespace Robotopia.ModManager.Tests
{
    internal static class ModServiceRegistryTests
    {
        public static void Run()
        {
            ...
            registry.RegisterFramework<ISceneCoordinator>("$robotopia.modmanager", framework);
            ...
            AssertThrows<InvalidOperationException>(
                () => registry.Register<ISceneCoordinator>("$robotopia.modmanager", new FakeSceneCoordinator()),
                "spoofing the framework owner must not replace the coordinator");
            registry.UnregisterOwner("$robotopia.modmanager");
            Assert(ReferenceEquals(registry.Get<ISceneCoordinator>(), framework),
        ...
        {
            var root = Path.Combine(Path.GetTempPath(), "RobotopiaOwnerRegistry-" + Guid.NewGuid().ToString("N"));
