...
{
    internal static class ManifestCompatibilityTests
    {
        internal static void Run(string root)
        {
            ...
            Console.WriteLine("ManifestCompatibilityTests passed.");
        }
        ...
        private static void TestAliasPrecedence()
        {
            var manifest = ValidManifest("compat.alias");
            manifest.SupportedGameVersionRange = "0.0.2227";
            manifest.GameVersionRange = "not-a-range";
            manifest.GameVersion = "9999";
            manifest.SupportedLoaderVersionRange = ">=0.2.0 <0.3.0";
            manifest.LoaderVersionRange = "not-a-range";
            manifest.SupportedSdkVersionRange = ">=0.1.0 <0.2.0";
            manifest.SdkVersionRange = "not-a-range";
            Assert(ManifestValidator.Validate(
                    manifest,
                    new ManifestValidationContext("0.0.2227", requireKnownGameVersion: true)).Count == 0,
                "canonical compatibility fields should win over malformed legacy aliases");

            manifest.SupportedGameVersionRange = string.Empty;
            Assert(ManifestValidator.Validate(
                    manifest,
                    new ManifestValidationContext("0.0.2227", requireKnownGameVersion: true))
                .Any(error => error.Contains("supportedGameVersionRange is invalid", StringComparison.Ordinal)),
                "gameVersionRange should be the second-precedence alias");

            manifest.GameVersionRange = string.Empty;
            manifest.GameVersion = "2227";
            Assert(ManifestValidator.Validate(
                    manifest,
                    new ManifestValidationContext("0.0.2227", requireKnownGameVersion: true))
                .All(error => !error.Contains("GameVersion", StringComparison.OrdinalIgnoreCase)),
                "numeric legacy gameVersion should use build-id normalization");
        }
        ...
        private static void TestRegistryAndInstallerThreadContext(string root)
        {
            ...
            paths.EnsureCreated();
            var package = Path.Combine(testRoot, "constrained.robotopiamod");
            Directory.CreateDirectory(testRoot);
            var manifest = ValidManifest("compat.threaded");
            manifest.SupportedGameVersionRange = "0.0.2227";
            using (var archive = ZipFile.Open(package, ZipArchiveMode.Create))
            {
                WriteEntry(archive, "robotopia.mod.json", JsonUtil.Serialize(manifest));
