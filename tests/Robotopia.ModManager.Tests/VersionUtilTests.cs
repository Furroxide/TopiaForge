...
namespace Robotopia.ModManager.Tests
{
    internal static class VersionUtilTests
    {
        ...
        {
            Assert(VersionUtil.TryParseRange("1") && VersionUtil.AllowsRange("1.0.0", "1"),
                "range syntax should preserve the legacy one-component shorthand");
            Assert(VersionUtil.TryParseRange("1.2") && VersionUtil.AllowsRange("1.2.0", "1.2"),
                "range syntax should preserve the legacy two-component shorthand");
            Assert(VersionUtil.TryParseRange(">=1 <2.1") &&
                   VersionUtil.AllowsRange("1.9.9", ">=1 <2.1") &&
                   !VersionUtil.AllowsRange("2.1.0", ">=1 <2.1"),
                "partial versions should normalize only inside ranges");
