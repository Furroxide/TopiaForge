...
namespace Robotopia.UgcLiveSync
{
    ...
    internal sealed class UgcLiveSyncService : IUgcLiveSyncService, IDisposable
    {
        ...
        public string SceneOwnerId { get; set; } = "robotopia.ugc.livesync";
        ...
        // Compatibility wrapper retained for callers/tests that used the old prefix probe. It now performs the
        // same strict UTF-8, complete structural validation used before applying a snapshot.
        internal static bool LooksLikeProjectJson(byte[] bytes)
        {
            return UgcSnapshotPayloadValidator.TryValidate(
                bytes,
                Math.Max(bytes?.LongLength ?? 0, DefaultMaxSnapshotBytes),
                out _);
        }
