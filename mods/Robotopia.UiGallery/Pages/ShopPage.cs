...
{
    /// <summary>
    /// The shop pane and window (SDK ShopItem/ShopWallet + QwShopPane): affordability
    ...
    internal static class ShopPage
    {
        public static void Build(QwContainer page)
        {
            ...
            var status = page.Label("Nothing bought yet.", QwTextStyle.Caption).Tone(QwTone.Muted);
            var pane = new QwShopPane(page, catalog, wallet, new QwShopPaneOptions { CurrencyLabel = "CREDITS" })
            {
            ...
            var controls = page.Row(QwGap.Sm);
            controls.Button("GRANT 500", () => wallet.Earn(500), QwButtonStyle.Outline);
