...
{
    /// <summary>
    /// QwUi-owned companion surface for gamemode pause actions. The vanilla pause bridge only inspects and
    /// rewires the game's existing exit button; every Robotopia-created visual remains inside the UI kit.
    /// </summary>
    internal sealed class PauseActionOverlay : IDisposable
    {
        ...
        private QwContainer? actionContent;
        ...
        private void EnsureWindow()
        {
            ...
            window = ui.Window("world-pause-actions", "WORLD ACTIONS", 380f, 310f, QwScheme.Paper);
            var scroll = window.Content.Scroll(QwGap.Sm, QwGap.Xs).FixedHeight(220f);
            ...
            for (var index = 0; index < actions.Count; index++)
            {
                ...
                actionContent.Button(
                    ...
                    action.Destructive ? QwButtonStyle.Danger : QwButtonStyle.Filled);
            }
        }
        ...
        private void Execute(WorldPauseAction action)
        {
            ...
            {
                action.Callback();
                ui.Toast(action.Label + " complete.", QwTone.Success);
            }
            catch (Exception ex)
            {
                logger.Warn("Worlds pause action '" + action.Id + "' failed: " + ex.Message);
                ui.Toast(action.Label + " failed. Check diagnostics.", QwTone.Danger);
                return;
            }
